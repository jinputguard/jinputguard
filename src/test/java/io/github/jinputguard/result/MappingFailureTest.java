...
class MappingFailureTest {
	...
	class GetMessage {
		...
		void nominal() {
			...
			var failure = new MappingFailure(cause).atPath(Path.createPropertyPath("myVal"));
	...
	class Equality {
		...
		void test_equals_and_hash_code_onNull() {
			var failure = new MappingFailure(new Exception("cause"));
		...
		void test_equals_and_hash_code_onIdenticalObject() {
			var cause = new Exception("cause");
			var failure = new MappingFailure(cause);
			var otherFailure = new MappingFailure(cause);
		...
		void test_equals_and_hash_code_onNotSameValue() {
			var cause = new Exception("cause");
			var failure = new MappingFailure(new Exception("cause"));
			var otherFailure = new MappingFailure(new Exception("cause"));
			Assertions.assertThat(failure).isNotEqualTo(otherFailure);
		}

		@Test
		void test_equals_and_hash_code_onNotSameCause() {
			var failure = new MappingFailure(new Exception("cause1"));
			var otherFailure = new MappingFailure(new Exception("cause2"));
			Assertions.assertThat(failure).isNotEqualTo(otherFailure);
		}
